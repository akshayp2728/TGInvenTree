...
function yesNoLabel(value, options={}) {
    ...
    if (toBool(value)) {
        text = options.pass || 'YES';
        color = 'bg-success';
    } else {
        text = options.fail || 'NO';
        color = 'bg-warning';
    }
...
function deleteButton(url, text='Supprimer') {
...
function setupNotesField(element, url, options={}) {
    ...
    const mde = new EasyMDE({
        ...
        imageUploadFunction: function(imageFile, onSuccess, onError) {
            ...
            inventreeFormDataUpload('/api/notes-image-upload/', form_data, {
                success: function(response) {
...
function renderClipboard(s, prepend=false) {
    ...
    let clipString = `<span class="d-none d-xl-inline"><button class="btn clip-btn" type="button" data-bs-toggle='tooltip' title='copy to clipboard'><em class="fas fa-copy"></em></button></span>`;
