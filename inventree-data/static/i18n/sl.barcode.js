...
function makeBarcodeInput(placeholderText='', hintText='') {
    ...
    placeholderText = placeholderText || 'Scan barcode data here using barcode scanner';

    hintText = hintText || 'Enter barcode data';
    ...
    <div class='form-group'>
        ...
        <div class='controls'>
            <div class='input-group'>
                ...
                <button title='Scan barcode using connected webcam' id='barcode_scan_btn' type='button' class='btn btn-secondary' onclick='onBarcodeScanClicked()' style='display: none;'>
                    ${makeIcon('fa-camera')}
                </button>
            </div>
...
function makeNotesField(options={}) {
    ...
    var tooltip = options.tooltip || 'Enter optional notes for stock transfer';
    var placeholder = options.placeholder || 'Enter notes';
            ...
            error: function(xhr) {
                ...
                case 400:
                    ...
                    if (options.onError400) {
                        ...
                        showBarcodeMessage(modal, data.error || 'Napaka na strežniku');
                    }
                    break;
                default:
...
function showInvalidResponseError(modal, response, status) {
    showBarcodeMessage(
        modal,
        `Invalid server response<br>Status: '${status}'`
    );
}
...
function barcodeDialog(title, options={}) {
    ...
    var details = options.details || 'Scan barcode data';
...
function barcodeScanDialog(options={}) {
    ...
    let title = options.title || 'Scan Barcode';
...
function unlinkBarcode(data, options={}) {
    ...
    var html = `<b>Unlink Barcode</b><br>`;

    html += 'This will remove the link to the associated barcode';
...
function barcodeCheckInStockItems(location_id, options={}) {
    ...
    function reloadTable() {
        ...
        items.forEach(function(item) {
            ...
            <tr pk='${item.pk}'>
                ...
                <td>${location_info}</td>
                <td>${item.quantity}</td>
                <td>${makeRemoveButton('button-item-remove', item.pk, 'Remove stock item')}</td>
            </tr>`;
        });
    ...
    barcodeDialog(
        ...
        {
            details: 'Scan stock item barcode to check in to this location',
            headerContent: table,
            preShow: function() {
                modalSetSubmitText(modal, 'Check In');
                modalEnable(modal, false);
                reloadTable();
            },
            onShow: function() {
            ...
            onSubmit: function() {
                ...
                if (entries.length == 0) {
                    showBarcodeMessage(modal, 'No barcode provided', 'warning');
                    return;
                }
            ...
            onScan: function(response) {
                if ('stockitem' in response) {
                    ...
                    inventreeGet(
                        ...
                        {},
                        {
                            success: function(stockitem) {
                                ...
                                if (duplicate) {
                                    showBarcodeMessage(modal, 'Stock Item already scanned', 'warning');
                                } else {

                                    if (stockitem.location == location_id) {
                                        showBarcodeMessage(modal, 'Stock Item already in this location');
                                        return;
                                    }
                                    ...
                                    showBarcodeMessage(modal, 'Added stock item', 'success');
                    ...
                    showBarcodeMessage(modal, 'Barcode does not match valid stock item', 'warning');
                }
            },
        }
    );
...
function barcodeCheckInStockLocations(location_id, options={}) {
    ...
    barcodeDialog(
        'Scan Stock Container Into Location',
        {
            details: 'Scan stock container barcode to check in to this location',
            ...
            onScan: function(response) {
                if ('stocklocation' in response) {
                    ...
                    showBarcodeMessage(modal, 'Barcode does not match valid stock location', 'warning');
                }
            }
        }
    );
                ...
                if ('stocklocation' in response) {
                    ...
                    inventreeGet(`/api/stock/location/${pk}/`, {}, {
                        ...
                        error: function() {
                            ...
                            showBarcodeMessage(
                                modal,
                                'Barcode does not match a valid location',
                                'warning',
                            );
                        }
                    });
