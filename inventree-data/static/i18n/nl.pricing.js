...
function getCurrencyConversionRates(cache=true) {
    ...
    inventreeGet('/api/currency/exchange/', {}, {
        async: false,
...
function calculateTotalPrice(dataset, value_func, currency_func, options={}) {
    ...
    if (!rates) {
        ...
        return `<span class='icon-red fas fa-exclamation-circle' title='Error fetching currency data'></span>`;
    }
...
function loadPartSupplierPricingTable(options={}) {
    ...
    table.inventreeTable({
        ...
        showColumns: false,
        formatNoMatches: function() {
            return 'No supplier pricing data available';
        },
        onLoadSuccess: function(data) {
            ...
            var graphLabels = Array.from(data, (x) => (`${x.part_detail.SKU} - Quantity ${x.quantity}`));
...
function loadPriceBreakTable(table, options={}) {
    ...
    table.inventreeTable({
        ...
        method: 'get',
        formatNoMatches: function() {
            return `No price break data available`;
        },
        queryParams: {
            part: options.part
        },
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    buttons += makeEditButton(`button-${name}-edit`, row.pk, `Edit ${human_name}`);
                    buttons += makeDeleteButton(`button-${name}-delete`, row.pk, `Verwijderen ${human_name}"`);
...
function loadPurchasePriceHistoryTable(options={}) {
    ...
    options.params.order_status = 30;
    ...
    table.inventreeTable({
        ...
        showColumns: false,
        formatNoMatches: function() {
            return 'No purchase history data available';
        },
        onLoadSuccess: function(data) {
...
function loadSalesPriceHistoryTable(options={}) {
    ...
    options.params.order_status = 20;
    ...
    table.inventreeTable({
        ...
        showColumns: false,
        formatNoMatches: function() {
            return 'No sales history data available';
        },
        onLoadSuccess: function(data) {
