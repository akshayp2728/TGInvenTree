    ...
    getApiEndpointOptions(url, function(OPTIONS) {
        ...
        case 'POST':
            if (canCreate(OPTIONS)) {
                ...
                showMessage('Action Prohibited', {
                    style: 'danger',
                    details: 'Create operation not allowed',
        ...
        case 'PATCH':
            if (canChange(OPTIONS)) {
                ...
                showMessage('Action Prohibited', {
                    style: 'danger',
                    details: 'Update operation not allowed',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'DELETE':
            if (canDelete(OPTIONS)) {
                ...
                showMessage('Action Prohibited', {
                    style: 'danger',
                    details: 'Delete operation not allowed',
                    icon: 'fas fa-user-times',
                });
        ...
        case 'GET':
            if (canView(OPTIONS)) {
                ...
                showMessage('Action Prohibited', {
                    style: 'danger',
                    details: 'View operation not allowed',
                    icon: 'fas fa-user-times',
                });
...
function insertConfirmButton(options) {
    ...
    var message = options.confirmMessage || 'Подтвердить';
...
function insertPersistButton(options) {
    ...
    var message = options.persistMessage || 'Keep this form open';
        ...
        if (field) {
            ...
            case 'decimal':
                if (!validateFormField(name, options)) {
                    ...
                    data_errors[name] = ['Enter a valid number'];
                }
                break;
            default:
                break;
...
function constructField(name, parameters, options={}) {
    ...
    if (extra) {
        ...
        if (!parameters.required && !options.hideClearButton) {
            html += `
            <span class='input-group-text form-clear' id='clear_${field_name}' title='Clear input'>
...
function selectImportFields(url, data={}, options={}) {
    ...
    var headers = `<tr><th>File Column</th><th>Field Name</th></tr>`;
