            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.has_allocated_stock) {
                    ...
                    <div class='alert alert-block alert-warning'>
                        Stock items have been allocated to this build order
                    </div>`;
                }
                ...
                if (opts.context.incomplete_outputs) {
                    html += `
                    <div class='alert alert-block alert-warning'>
                        There are incomplete outputs remaining for this build order
                    </div>`;
                }
    ...
    constructForm(`/api/build/${build_id}/finish/`, {
        ...
        preFormContent: function(opts) {
            ...
            if (ctx.allocated && ctx.remaining == 0 && ctx.incomplete == 0) {
                ...
                Build order is ready to be completed'
                </div>`;
            } else {
                ...
                if (ctx.incomplete > 0) {
                    ...
                    <strong>Build order has incomplete outputs</strong><br>
                    This build order cannot be completed as there are incomplete outputs
                    </div>`;
                } else {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    <strong>Build Order is incomplete</strong>
                    </div>
                    `;
                }

                if (!ctx.allocated) {
                    html += `<div class='alert alert-block alert-warning'>Required stock has not been fully allocated</div>`;
                }

                if (ctx.remaining > 0) {
                    html += `<div class='alert alert-block alert-warning'>Required build quantity has not been completed</div>`;
                }
            }
        ...
        {},
        {
            success: function(build) {
                ...
                inventreeGet(`/api/part/${build.part}/serial-numbers/`, {}, {
                    success: function(data) {
                        if (data.next) {
                            fields.serial_numbers.placeholder = `Next available serial number: ${data.next}`;
                        } else if (data.latest) {
                            fields.serial_numbers.placeholder = `Latest serial number: ${data.latest}`;
                        }
                    },
                    async: false,
                });
                ...
                if (options.trackable_parts) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        The Bill of Materials contains trackable parts.<br>
                ...
                if (trackable) {
                    html += `
                    <div class='alert alert-block alert-info'>
                        Trackable parts can have serial numbers specified<br>
                        Enter serial numbers to generate multiple single build outputs
                    </div>
                    `;
                }
...
function makeBuildOutputButtons(output_id, build_info, options={}) {
    ...
    if (options.has_tracked_lines) {
        ...
        html += makeIconButton(
            ...
            output_id,
            'Allocate stock items to this build output',
        );
        ...
        html += makeIconButton(
            ...
            output_id,
            'Deallocate stock from build output',
        );
    }
...
function deallocateStock(build_id, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-warning'>
    Are you sure you wish to deallocate the selected stock items from this build?
    </dvi>
    `;
...
function renderBuildOutput(output, options={}) {
    ...
    if (output.quantity == 1 && output.serial) {
        output_html += `Serial Number: ${output.serial}`;
    } else {
        output_html += `Quantity: ${output.quantity}`;
    ...
    buttons += makeRemoveButton('button-row-remove', pk, 'Remove row');
...
function completeBuildOutputs(build_id, outputs, options={}) {
    ...
    if (outputs.length == 0) {
        showAlertDialog(
            ...
            'At least one build output must be selected',
        );
        return;
    }
    ...
    var html = `
    <div class='alert alert-block alert-success'>
    Selected build outputs will be marked as complete
    </div>
...
function scrapBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Selected build outputs will be marked as scrapped
    <ul>
        <li>Scrapped output are marked as rejected</li>
        <li>Allocated stock items will no longer be available</li>
        <li>The completion status of the build order will not be adjusted</li>
    </ul>
    </div>
...
function deleteBuildOutputs(build_id, outputs, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-danger'>
    Selected build outputs will be deleted
    <ul>
    <li>Build output data will be permanently deleted</li>
    <li>Allocated stock items will be returned to stock</li>
    </ul>
    </div>
    <table class='table table-striped table-condensed' id='build-complete-table'>
        <thead>
...
function loadBuildOrderAllocationTable(table, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No build order allocations found';
        },
        columns: [
            {
                field: 'pk',
...
function loadBuildOutputTable(build_info, options={}) {
    ...
    setupFilterList('builditems', $(table), options.filterTarget || '#filter-list-incompletebuilditems', {
        ...
        singular_name: 'build output',
        plural_name: 'build outputs',
        custom_actions: [{
            label: 'buildoutput',
            icon: 'fa-tools',
            title: 'Build output actions',
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No active build outputs found';
        },
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.batch) {
                        text += ` <small>(Batch: ${row.batch})</small>`;
                    }
...
function allocateStockToBuild(build_id, line_items, options={}) {
    ...
    if (table_entries.length == 0) {
        ...
        showAlertDialog(
            'All Parts Allocated',
            'All selected parts have been fully allocated',
        );
    ...
    constructForm(`/api/build/${build_id}/allocate/`, {
        ...
        fields: {},
        preFormContent: html,
        title: 'Allocate Stock Items to Build Order',
...
function autoAllocateStockToBuild(build_id, bom_items=[], options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
    <strong>Automatic Stock Allocation</strong><br>
    Stock items will be automatically allocated to this build order, according to the provided guidelines:
    <ul>
        <li>If a location is specified, stock will only be allocated from that location</li>
        <li>If stock is considered interchangeable, it will be allocated from the first location it is found</li>
        <li>If substitute stock is allowed, it will be used where stock of the primary part cannot be found</li>
    </ul>
    </div>
    `;
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.overdue) {
                        html += makeIconBadge('fa-calendar-times icon-red', 'Build order is overdue');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.responsible_detail.label == 'csoport') {
    ...
    sub_table.bootstrapTable({
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    let buttons = '';
                    buttons += makeEditButton('button-allocation-edit', row.pk, 'Edit stock allocation');
                    buttons += makeDeleteButton('button-allocation-delete', row.pk, 'Delete stock allocation');
...
function loadBuildLineTable(table, build_id, options={}) {
    ...
    if (!options.data) {
        setupFilterList('buildlines', $(table), filterTarget, {
            labels: {
                url: '/api/label/buildline/',
                key: 'line',
            },
            singular_name: 'build line',
            plural_name: 'build lines',
        });
    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.allow_variants) {
                        html += makeIconBadge('fa-sitemap', 'Variant stock allowed');
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.bom_item_detail.consumable) {
                        icons += `<span class='fas fa-info-circle icon-blue float-right' title='Consumable item'></span>`;
                    } else {
                        if (available < (row.quantity - row.allocated)) {
                            icons += makeIconBadge('fa-times-circle icon-red', 'Insufficient stock available');
                        } else {
                            icons += makeIconBadge('fa-check-circle icon-green', 'Sufficient stock available');
                        }

                        if (available <= 0) {
                            icons += `<span class='badge rounded-pill bg-danger'>No Stock Available</span>`;
                        } else {
                            let extra = '';
                            if ((row.available_substitute_stock > 0) && (row.available_variant_stock > 0)) {
                                extra = 'Includes variant and substitute stock';
                            } else if (row.available_variant_stock > 0) {
                                extra = 'Includes variant stock';
                            } else if (row.available_substitute_stock > 0) {
                                extra = 'Includes substitute stock';
                            }
                    ...
                    if (row.on_order && row.on_order > 0) {
                        icons += makeIconBadge('fa-shopping-cart', `On Order: ${formatDecimal(row.on_order)}`);
                    }
                ...
                formatter: function(value, row) {
                    ...
                    if (row.allocated < row.quantity) {
                        ...
                        if (row.part_detail.assembly) {
                            buttons += makeIconButton('fa-tools icon-blue', 'button-build', pk, 'Build stock');
                        }

                        // Add a button to "purchase" stock for this line
                        if (row.part_detail.purchaseable) {
                            buttons += makeIconButton('fa-shopping-cart icon-blue', 'button-buy', pk, 'Order stock');
                        }

                        // Add a button to "allocate" stock for this line
                        buttons += makeIconButton('fa-sign-in-alt icon-green', 'button-allocate', pk, 'Allocate stock');
                    }

                    if (row.allocated > 0) {
                        buttons += makeRemoveButton('button-unallocate', pk, 'Remove stock allocation');
                    }
