...
function partGroups() {
    ...
    return {
        ...
        create: {
            title: 'Part Creation Options',
            collapsible: true,
        },
        duplicate: {
            title: 'Part Duplication Options',
            collapsible: true,
        },
        initial_stock: {
            title: 'Initial Stock',
...
function categoryFields(options={}) {
    let fields = {
        parent: {
            help_text: 'Parent part category',
            required: false,
        },
        name: {},
        description: {},
        ...
        structural: {},
        icon: {
            help_text: `Icon (optional) - Explore all available icons on <a href="https://fontawesome.com/v5/search?s=solid" target="_blank" rel="noopener noreferrer">Font Awesome</a>.`,
...
function createPartCategory(options={}) {
    ...
    constructForm('/api/part/category/', {
        fields: fields,
        method: 'POST',
        title: 'Create Part Category',
        follow: true,
        persist: true,
        persistMessage: 'Create new category after this one',
        successMessage: 'Part category created'
    });
}
...
function deletePartCategory(pk, options={}) {
    ...
    var subChoices = [
        {
            value: 0,
            display_name: 'Move to parent category',
        },
        {
            value: 1,
            display_name: 'Verwijderen',
        }
    ];
    ...
    constructForm(url, {
        ...
        fields: {
            'delete_parts': {
                label: 'Action for parts in this category',
                choices: subChoices,
                type: 'choice'
            },
            'delete_child_categories': {
                label: 'Action for child categories',
                choices: subChoices,
                type: 'choice'
            },
        },
...
function createPart(options={}) {
    ...
    constructForm('/api/part/', {
        method: 'POST',
        fields: partFields(options),
        groups: partGroups(),
        title: 'Create Part',
        persist: true,
        persistMessage: 'Create another part after this one',
...
function duplicatePart(pk, options={}) {
    ...
    var title = 'Duplicate Part';

    if (options.variant) {
        title = 'Create Part Variant';
    }
    ...
    inventreeGet(`/api/part/${pk}/`, {}, {
        success: function(part) {
            if (part.active) {
                showAlertDialog(
                    'Active Part',
                    'Part cannot be deleted as it is currently active',
                    {
                        alert_style: 'danger',
                    }
                );
            ...
            Deleting this part cannot be reversed
            <ul>
            <li>Any stock items for this part will be deleted</li>
            <li>This part will be removed from any Bills of Material</li>
            <li>All manufacturer and supplier information for this part will be deleted</li>
            </div>`;
                ...
                },
                {
                    ...
                    success: function(response) {
                        if (response.starred) {
                            ...
                            $(options.button).attr('title', 'You are subscribed to notifications for this item');

                            showMessage('You have subscribed to notifications for this item', {
                                style: 'success',
                            });
                        } else {
                            $(options.button).removeClass('fas fa-bell icon-green').addClass('fa fa-bell-slash');
                            $(options.button).attr('title', 'Subscribe to notifications for this item');

                            showMessage('You have unsubscribed to notifications for this item', {
                                style: 'warning',
                            });
                        }
                    }
...
function validateBom(part_id, options={}) {
    ...
    Validating the BOM will mark each line item as valid
    </div>
    `;
    ...
    constructForm(`/api/part/${part_id}/bom-validate/`, {
        ...
        preFormContent: html,
        title: 'Validate Bill of Materials',
        reload: options.reload,
        onSuccess: function(response) {
            showMessage('Validated Bill of Materials');
        }
    });
}
...
function partStockLabel(part, options={}) {
    ...
    if (part.total_in_stock) {
        ...
        elements.push(`No Stock`);
    }

    // Check for items on order
    if (part.ordering) {
        elements.push(`On Order: ${part.ordering}`);
    }

    // Check for items being built
    if (part.building) {
        elements.push(`Building: ${part.building}`);
    }
    ...
    if (units && !options.no_units) {
        output += `<span class='badge rounded-pill text-muted bg-muted ${classes}'>Unit: ${units}</span> `;
    }
...
function makePartIcons(part) {
    ...
    if (part.trackable) {
        html += makeIconBadge('fa-directions', 'Trackable part');
    }

    if (part.virtual) {
        html += makeIconBadge('fa-ghost', 'Virtual part');
    }
    ...
    if (part.starred) {
        html += makeIconBadge('fa-bell icon-green', 'Subscribed part');
    }

    if (part.salable) {
        html += makeIconBadge('fa-dollar-sign', 'Salable part');
    }

    if (!part.active) {
        html += `<span class='badge badge-right rounded-pill bg-warning'>Inactive</span> `;
    }
...
function generateStocktakeReport(options={}) {
    ...
    Schedule generation of a new stocktake report. Once complete, the stocktake report will be available for download.
    </div>
    `;
    ...
    constructForm(
        ...
        {
            method: 'POST',
            title: 'Generate Stocktake Report',
            preFormContent: content,
            fields: fields,
            onSuccess: function(response) {
                showMessage('Stocktake report scheduled', {
                    style: 'success',
                });
            }
        }
...
function loadPartStocktakeTable(partId, options={}) {
    ...
    $(table).inventreeTable({
        ...
        formatNoMatches: function() {
            return 'No stocktake information available';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (options.allow_edit) {
                        html += makeEditButton('button-edit-stocktake', row.pk, 'Edit Stocktake Entry');
                    }

                    if (options.allow_delete) {
                        html += makeDeleteButton('button-delete-stocktake', row.pk, 'Delete Stocktake Entry');
                    }
        ...
        },
        {
            ...
            formatter: function(value, row) {
                ...
                if (row.variant_stock > 0) {
                    text = `<em>${text}</em>`;
                    text += `<span title='Includes variant stock' class='fas fa-info-circle float-right icon-blue'></span>`;
                }
        ...
        template: {
            ...
            onEdit: function(value, name, field, opts) {
                ...
                if (value) {
                    ...
                    inventreeGet(`/api/part/parameter/template/${value}/`, {}, {
                        async: false,
                        success: function(response) {
...
function createPartParameter(part_id, options={}) {
    ...
    constructForm('/api/part/parameter/', options);
}
...
function editPartParameter(param_id, options={}) {
    ...
    constructForm(`/api/part/parameter/${param_id}/`, options);
}
...
function loadPartParameterTemplateTable(table, options={}) {
    ...
    $(table).inventreeTable({
        url: '/api/part/parameter/template/',
        original: params,
        queryParams: filters,
        sortable: true,
        ...
        formatNoMatches: function() {
            return 'No part parameter templates found';
        },
        columns: [
            {
                field: 'pk',
            ...
            },
            {
                formatter: function(value, row, index, field) {
                    ...
                    buttons += makeEditButton('template-edit', row.pk, 'Edit Template');
                    buttons += makeDeleteButton('template-delete', row.pk, 'Delete Template');
    ...
    $(table).on('click', '.template-edit', function() {
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                ...
                title: 'Edit Part Parameter Template',
                refreshTable: table,
            }
        );
    });
    ...
    $(table).on('click', '.template-delete', function() {
        ...
        var html = `
        <div class='alert alert-block alert-danger'>
            Any parameters which reference this template will also be deleted
        </div>`;
        ...
        constructForm(
            `/api/part/parameter/template/${pk}/`,
            {
                method: 'DELETE',
                preFormContent: html,
                title: 'Delete Part Parameter Template',
                refreshTable: table,
            }
        );
    });
                ...
                formatter: function(value, row) {
                    ...
                    if (row.supplier_part_detail.pack_quantity_native != 1.0) {
                        ...
                        data += makeIconBadge(
                            ...
                            `Pack Quantity: ${row.pack_quantity} - Total Quantity: ${total}`
                        );
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (value > 0 && row.supplier_part_detail.pack_quantity_native != 1.0) {
                        let total = value * row.supplier_part_detail.pack_quantity_native;
                        data += `<span class='fas fa-info-circle icon-blue float-right' title='Pack Quantity: ${row.pack_quantity} - Total Quantity: ${total}'></span>`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.received >= row.quantity) {
                        // Already received
                        return `<span class='badge bg-success rounded-pill'>Received</span>`;
                    } else if (row.order_detail && row.order_detail.status == 20) {
                        let html = '';
                        var pk = row.pk;

                        html += makeIconButton('fa-sign-in-alt', 'button-line-receive', pk, 'Receive line item');
...
function loadRelatedPartsTable(table, part_id, options={}) {
    ...
    var columns = [
        ...
        },
        {
            ...
            formatter: function(value, row) {
                let html = '';
                html += makeDeleteButton('button-related-delete', row.pk, 'Delete part relationship');
        ...
        onPostBody: function() {
            $(table).find('.button-related-delete').click(function() {
                ...
                constructForm(`/api/part/related/${pk}/`, {
                    method: 'DELETE',
                    title: 'Delete Part Relationship',
                    refreshTable: table,
                });
            });
        },
...
function partGridTile(part) {
    ...
    if (!part.in_stock) {
        stock = `<span class='badge rounded-pill bg-danger'>No Stock</span>`;
    } else if (!part.unallocated_stock) {
        stock = `<span class='badge rounded-pill bg-warning'>No Stock</span>`;
    }

    rows += `<tr><td><b>Stock</b></td><td>${stock}</td></tr>`;

    if (part.ordering) {
        rows += `<tr><td><b>On Order</b></td><td>${part.ordering} ${units}</td></tr>`;
    }

    if (part.building) {
        rows += `<tr><td><b>Building</b></td><td>${part.building} ${units}</td></tr>`;
    }
...
function setPartCategory(data, options={}) {
    ...
    var html = `
    <div class='alert alert-block alert-info'>
        Set the part category for the selected parts
    </div>
    `;

    constructForm('/api/part/change_category/',{
        title: 'Set Part Category',
        method: 'POST',
        preFormContent: html,
        fields: {
            category: {},
...
function loadPartCategoryTable(table, options) {
    ...
    table.inventreeTable({
        ...
        url: options.url || '/api/part/category/',
        queryParams: filters,
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row) {
                        ...
                        if (row.subRequested) {
                            ...
                            html += `
                                <a href='#' pk='${row.pk}' class='load-sub-category'>
                                    <span class='fas fa-sync-alt' title='Load Subcategories'></span>
                                </a> `;
                        }
                    }
                    ...
                    if (row.starred) {
                        html += makeIconBadge('fa-bell icon-green', 'Subscribed category');
                    }
...
function loadPartTestTemplateTable(table, options) {
    ...
    table.inventreeTable({
        method: 'get',
        formatNoMatches: function() {
            return 'No test templates matching query';
        },
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.part == part) {
                        ...
                        html += makeEditButton('button-test-edit', pk, 'Edit test result');
                        html += makeDeleteButton('button-test-delete', pk, 'Delete test result');

                        return wrapButtons(html);
                    } else {
                        var text = 'This test is defined for a parent part';
        ...
        onPostBody: function() {
            ...
            table.find('.button-test-edit').click(function() {
                ...
                constructForm(url, {
                    ...
                    title: 'Edit Test Result Template',
                    onSuccess: function() {
                        table.bootstrapTable('refresh');
                    },
                });
            ...
            table.find('.button-test-delete').click(function() {
                ...
                constructForm(url, {
                    method: 'DELETE',
                    title: 'Delete Test Result Template',
                    refreshTable: table,
                });
            });
        }
...
function loadPartSchedulingChart(canvas_id, part_id) {
        ...
        {},
        {
            ...
            success: function(response) {
                ...
                for (var idx = 0; idx < response.length; idx++) {
                    ...
                    if (date == null) {
                        date_string = '<em>No date specified</em>';
                        date_string += makeIconBadge('fa-exclamation-circle icon-red', 'No date specified');
                    } else if (date < today) {
                        date_string += makeIconBadge('fa-exclamation-circle icon-yellow', 'Specified date is in the past');
                    }

                    var quantity_string = entry.quantity + entry.speculative_quantity;

                    if (entry.speculative_quantity != 0) {
                        quantity_string += makeIconBadge('fa-question-circle icon-blue', 'Speculative');
                    }
    ...
    if (n_entries < 1) {
        ...
        var message = `
        <div class='alert alert-block alert-info'>
            No scheduling information available for this part.
        </div>`;

        if (was_error) {
            message = `
                <div class='alert alert-block alert-danger'>
                    Error fetching scheduling information for this part.
                </div>
            `;
        }
    ...
    var data = {
        datasets: [
            {
                label: 'Scheduled Stock Quantities',
                ...
                tooltip: {
                    callbacks: {
                        ...
                        afterLabel: function(item) {
                            ...
                            return `Quantity: ${item.raw.y}${delta}`;
                        }
                    }
                },
                legend: {
