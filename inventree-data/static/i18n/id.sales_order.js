            ...
            if (!allocations || allocations.length == 0) {
                ...
                allocations.forEach(function(allocation) {
                    ...
                    if (allocation.serial) {
                        stock = `Serial Number: ${allocation.serial}`;
                    } else {
        ...
        `/api/order/so/${order_id}/complete/`,
        {
            ...
            preFormContent: function(opts) {
                ...
                if (opts.context.pending_shipments) {
                    html += `
                    <div class='alert alert-block alert-danger'>
                    Order cannot be completed as there are incomplete shipments<br>
                    </div>`;
                }
        ...
        columns: [
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    let text = `Quantity: ${row.quantity}`;

                    if (item && item.serial != null && row.quantity == 1) {
                        text = `Serial Number: ${item.serial}`;
                    }
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        ...
                        // Location specified
                        return renderLink(
                            row.location_detail.pathstring || 'Location',
            ...
            },
            {
                ...
                formatter: function(value, row, index, field) {
                    ...
                    if (row.shipment_date) {
                        html += `<span class='badge bg-success badge-right'>Shipped</span>`;
                    } else {
                ...
                formatter: function(value, row) {
                    ...
                    if (required > 0) {
                        if (available >= required) {
                            ...
                            html += makeIconBadge('fa-times-circle icon-red', 'Insufficient stock available');
                        }
                    }
