...
function constructBomUploadTable(data, options={}) {
    ...
    function constructRow(row, idx, fields) {
        ...
        buttons += makeInfoButton('button-row-data', idx, 'Display row data');
        buttons += makeRemoveButton('button-row-remove', idx, 'Remove row');
    ...
    constructFormBody({}, {
        ...
        fields: {
            ...
            cascade: {
                ...
                help_text: 'Include BOM data for subassemblies',
                type: 'boolean',
                value: inventreeLoad('bom-export-cascading', true),
            },
            levels: {
                label: 'Levels',
                help_text: 'Select maximum number of BOM levels to export (0 = all levels)',
                type: 'integer',
                value: 0,
                required: true,
                min_value: 0,
            ...
            substitute_part_data: {
                label: 'Include Alternative Parts',
                help_text: 'Include alternative parts in exported BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-substitute_part_data', false),
            },
            parameter_data: {
                label: 'Include Parameter Data',
                help_text: 'Include part parameter data in exported BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-parameter_data', false),
            },
            stock_data: {
                label: 'Include Stock Data',
                help_text: 'Include part stock data in exported BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-stock_data', false),
            },
            manufacturer_data: {
                label: 'Include Manufacturer Data',
                help_text: 'Include part manufacturer data in exported BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-manufacturer_data', false),
            },
            supplier_data: {
                label: 'Include Supplier Data',
                help_text: 'Include part supplier data in exported BOM',
                type: 'boolean',
                value: inventreeLoad('bom-export-supplier_data', false),
            },
            pricing_data: {
                label: 'Include Pricing Data',
                help_text: 'Include part pricing data in exported BOM',
                type: 'boolean',
...
function bomSubstitutesDialog(bom_item_id, substitutes, options={}) {
    ...
    function renderSubstituteRow(substitute) {
        ...
        buttons += makeRemoveButton('button-row-remove', pk, 'Remove substitute part');
    ...
    <div class='alert alert-success alert-block'>
        Select and add a new substitute part using the input below
    </div>
    `;
    ...
    function addRemoveCallback(modal, element) {
        $(modal).find(element).click(function() {
            ...
            Are you sure you wish to remove this substitute part link?
            </div>
            `;

            constructForm(`/api/bom/substitute/${pk}/`, {
                method: 'DELETE',
                title: 'Remove Substitute Part',
                preFormContent: pre,
                confirm: true,
                onSuccess: function() {
    ...
    constructForm('/api/bom/substitute/', {
        ...
        preFormContent: html,
        closeText: 'بستن',
        submitText: 'Add Substitute',
        title: 'Edit BOM Item Substitutes',
...
function deleteBomItems(items, options={}) {
    ...
    All selected BOM items will be deleted
    </div>
    ...
    constructForm('/api/bom/', {
        method: 'DELETE',
        multi_delete: true,
        title: 'Delete selected BOM items?',
        form_data: {
            items: ids,
        },
        preFormContent: html,
...
function loadBomTable(table, options={}) {
    ...
    cols.push(
        {
            ...
            formatter: function(value, row) {
                    ...
                    if (row.sub_assembly_received) {
                        ...
                        html += `
                            <a href='#' pk='${row.pk}' class='load-sub-assembly' id='load-sub-assembly-${row.pk}'>
                                <span class='fas fa-sync-alt' title='Load BOM for subassembly'></span>
                            </a> `;
                    }
                }
                ...
                if (row.substitutes && row.substitutes.length > 0) {
                    html += makeIconBadge('fa-exchange-alt', 'Substitutes Available');
                }
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (row.consumable) {
                text += ` <small>(Consumable)</small>`;
            }

            if (row.optional) {
                text += ' <small>(Optional)</small>';
            }
    ...
    cols.push({
        ...
        footerFormatter: function(data) {
            ...
            if (any_pricing) {
                ...
                let html = '<em>No pricing available</em>';
    ...
    cols.push({
        ...
        formatter: function(value, row) {
            ...
            if (available_stock <= 0) {
                text += makeIconBadge('fa-times-circle icon-red', 'No Stock Available');
            } else {
                var extra = '';
        ...
        cols.push({
            ...
            formatter: function(value, row) {
                ...
                if (row.part == options.parent_id) {
                    ...
                    var bValidate = makeIconButton('fa-check-circle icon-green', 'bom-validate-button', row.pk, 'Validate BOM Item');

                    var bValid = makeIconButton('fa-check-double icon-green', 'bom-valid-button', row.pk, 'This line has been validated', {disabled: true});

                    var bSubs = makeIconButton('fa-exchange-alt icon-blue', 'bom-substitutes-button', row.pk, 'Edit substitute parts');

                    var bEdit = makeEditButton('bom-edit-button', row.pk, 'Edit BOM Item');

                    var bDelt = makeDeleteButton('bom-delete-button', row.pk, 'Delete BOM Item');
            ...
            footerFormatter: function(data) {
                return `
                <button class='btn btn-success float-right' type='button' title='Add BOM Item' id='bom-item-new-footer'>
                    ${makeIcon('fa-plus-circle')} Add BOM Item
                </button>
                `;
            }
        });
            ...
            },
            {
                ...
                formatter: function(value, row) {
                    ...
                    if (row.parent && row.parent != 'top-level-item') {
                        html += ` <em>(Inherited from parent BOM)</em>`;
                    }
