...
function createNewModal(options={}) {
    ...
    if (!options.hideCloseButton) {
        buttons += `<button type='button' class='btn btn-secondary' id='modal-form-close' data-bs-dismiss='modal'>Cancel</button>`;
    }

    // Add in a "submit" button
    if (!options.hideSubmitButton) {
        buttons += `<button type='button' class='btn btn-${submitClass}' id='modal-form-submit'>Отправить</button>`;
    }
    ...
    modalSetTitle(modal_name, options.title || 'Form Title');
    modalSetSubmitText(modal_name, options.submitText || 'Отправить');
    modalSetCloseText(modal_name, options.closeText || 'Cancel');
        ...
        <div class='panel'>
            <div class='panel panel-heading'>
                <div class='panel-title'>
                    <a data-bs-toggle='collapse' href="#collapse-error-info">Show Error Information</a>
                </div>
            </div>
...
function showQuestionDialog(title, content, options={}) {
    ...
    options.title = title;
    options.submitText = options.accept_text || 'Accept';
    options.closeText = options.cancel_text || 'Cancel';
...
function openModal(options) {
    ...
    var submit_text = options.submit_text || 'Отправить';
    var close_text = options.close_text || 'Закрыть';
